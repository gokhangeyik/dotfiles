...
_GokkoNvim.activate_python_env = function(env_name)
  ...
  for _, client in ipairs(vim.lsp.get_clients()) do
    if client.name == "basedpyright" then
      ...
      -- Restart client safely for Neovim 0.11+
      local client_id = client.id
      vim.schedule(function()
        vim.lsp.buf_detach_client(0, client_id)
        vim.lsp.stop_client(client_id, true)
        vim.cmd("sleep 100m") -- Give a small delay for client to fully stop
        vim.cmd("edit") -- Reload the buffer to trigger client restart
      end)
      break
    end
  end
